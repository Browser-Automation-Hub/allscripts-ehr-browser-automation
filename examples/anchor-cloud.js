/**
 * anchor-cloud.js — AnchorBrowser cloud example for Allscripts EHR
 *
...
const { login_allscripts, search_patient } = require('../src/actions');
...
async function main() {
  ...
  try {
    log('Starting Allscripts EHR automation (AnchorBrowser Cloud mode)');
    ...
    const result = await withAnchorBrowser(async (page) => {
      ...
      const data1 = await login_allscripts(page, {});
      log('Action 1 result:', data1);

      const data2 = await search_patient(page, {});
