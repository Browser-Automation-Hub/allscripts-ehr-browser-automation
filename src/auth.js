/**
 * auth.js — Authentication handler for Allscripts EHR
 * Supports: SSO (LDAP / AD / SAML), MFA (Duo / RSA)
 *
...
const BASE_URL = process.env.ALLSCRIPTS_URL || 'https://your-instance.allscripts.com/login';
const USERNAME = process.env.ALLSCRIPTS_USERNAME;
const PASSWORD = process.env.ALLSCRIPTS_PASSWORD;
...
async function performLogin(page) {
  if (!USERNAME || !PASSWORD) {
    throw new Error(`Set ALLSCRIPTS_USERNAME and ALLSCRIPTS_PASSWORD in .env`);
  }

  console.log('Logging in to Allscripts EHR...');
  await page.goto(BASE_URL, { waitUntil: 'networkidle2' });
