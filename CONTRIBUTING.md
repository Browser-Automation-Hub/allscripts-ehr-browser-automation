# Contributing to Allscripts EHR Browser Automation
...
- **Broken selectors**: Allscripts EHR updates its UI periodically. If a selector breaks, open an issue with the current selector and what it should be.
...
2. Make your changes
3. Test your changes against a real Allscripts EHR instance (if you have access)
...
async function my_new_action(page, opts = {}) {
  ...
  return retry(async () => {
    ...
    await page.goto(`${process.env.ALLSCRIPTS_URL}/path/to/section`);
...
When you find working selectors for Allscripts EHR, document them:

```javascript
// Allscripts EHR selectors (verified YYYY-MM-DD)
...
Allscripts EHR updates its UI periodically. When selectors break:
